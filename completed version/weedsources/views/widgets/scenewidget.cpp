...
void SceneWidget::paintEvent(QPaintEvent*)
{
    ...
    for (int row = 0; row < gameFramework->getRows(); row++)
    {
        for (int col = 0; col < gameFramework->getCols(); col++)
        {
            MineUnit* unit = gameFramework->getUnit(row, col);

            int x = SceneProperties::MARGIN + col * UnitProperties::SIZE;
            int y = SceneProperties::MARGIN + row * UnitProperties::SIZE;
